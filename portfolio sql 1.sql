...
SELECT AVG(DATEDIFF(end_date, start_date)) AS avg_relocation_days 
FROM customer_nodes;
...
SELECT AVG(deposit_count) AS avg_deposit_count, AVG(total_amount) AS avg_deposit_amount 
FROM (SELECT customer_id, COUNT(*) AS deposit_count, SUM(amount) AS total_amount 
      FROM customer_transactions 
      GROUP BY customer_id) AS customer_deposits;

-- 3. For each month - how many Data Bank customers make more than 1 deposit and either 1 purchase or 1 withdrawal in a single month?
SELECT YEAR(txn_date) AS year, MONTH(txn_date) AS month, customer_id 
FROM customer_transactions 
...
HAVING SUM(CASE WHEN txn_type = 'Deposit' THEN 1 ELSE 0 END) > 1 AND 
       (SUM(CASE WHEN txn_type = 'Purchase' THEN 1 ELSE 0 END) = 1 OR 
...
SELECT customer_id, 
       ...
       LAST_VALUE(txn_amount) OVER(PARTITION BY customer_id, MONTH(txn_date), YEAR(txn_date) ORDER BY txn_date RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS month_end_balance
FROM customer_transactions
...
FROM (SELECT customer_id, txn_date,
             SUM(txn_amount) OVER(PARTITION BY customer_id ORDER BY txn_date) AS running_balance
      FROM customer_transactions) AS balances
GROUP BY customer_id;

-- Using all of the data available - how much data would have been required for each option on a monthly basis?
-- Option 1: Data provision based on the end-of-month balance could use the sums of month_end_balance for each customer.
